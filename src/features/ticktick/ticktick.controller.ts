import { Controller, Get, Post, Query, Req, Res, HttpException, HttpStatus, Logger, UseGuards } from '@nestjs/common';
  ...
  async callback(@Query('code') code: string, @Res() res: Response) {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.TICKTICK_OAUTH_URL}/oauth/token`,
        new URLSearchParams({
          client_id: clientId,
      ...
      const detail = isTimeout
        ? 'Connection to ticktick.com timed out.'
        : err.response?.data ? JSON.stringify(err.response.data) : err.message;
      this.logger.error(`TickTick OAuth error: ${detail}`);
